sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/model/json/JSONModel", "sap/m/MessageBox", "sap/ui/core/Fragment",
	...
	"use strict";
	return e.extend("dksh.connectclient.InventoryStck.controller.inventory", {
		formatter: o,
		...
		_setDefaultSalesOrg: function () {
			...
			var oFilter = new sap.ui.model.Filter({
				filters: [
					new sap.ui.model.Filter("Language", sap.ui.model.FilterOperator.EQ, lang),
					new sap.ui.model.Filter("Salesorg", sap.ui.model.FilterOperator.EQ, this.salesOrgDataAccess)
				],
				and: true
			});
			filters.push(oFilter);
		...
		_getUser: function () {
			...
			this._doAjax(e, "GET", "", true).then(e => {
				...
				this._getPersonalizationDetails("keyStkLotView");
			}, e => {
				t.close();
		...
		onPressDetailEndStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryStck.Fragments.EndingStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
		...
		onPressDetailExpStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryStck.Fragments.ExpiryStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailMatMov: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryStck.Fragments.MatMovPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailATP: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryStck.Fragments.ATPPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onTabSelection: function (e) {
			...
			this.selectedTab = "keyStkLotView";
		...
		onPersonlizationClose: function () {
			...
			if (!this.FilterPersonalization) {
				this.FilterPersonalization = sap.ui.xmlfragment("dksh.connectclient.InventoryStck.Fragments.FilterPersonalization", this);
			...
			e._getPersonalizationDetails("keyStkLotView")
		},
		...
		valueHelpRequestPlant: function (e) {
			...
			if (t.plantDataAccess === "No Access") {
				...
				var t = this;
				if (!t.plantFrag) {
					t.plantFrag = sap.ui.xmlfragment("dksh.connectclient.InventoryStck.Fragments.plant", t);
		...
		valueHelpRequestMaterialGrp4: function (e) {
			...
			if (t.materialGroup4DataAccess === "No Access") {
				...
				if (!t.materialGroup4) {
					t.materialGroup4 = sap.ui.xmlfragment("dksh.connectclient.InventoryStck.Fragments.materialGroup4", t);
		...
		valueHelpRequestMaterialGrp: function (e) {
			...
			if (t.materialGroupDataAccess === "No Access") {
				...
				if (!t.MaterialGroup) {
					t.MaterialGroup = sap.ui.xmlfragment("dksh.connectclient.InventoryStck.Fragments.MaterialGroup", t);
		...
		valueHelpRequestStoLoc: function (e) {
			...
			if (l.SLOCDataAccess === "No Access") {
				...
				l.StorageLocation = sap.ui.xmlfragment("dksh.connectclient.InventoryStck.Fragments.StorageLocation", l);
			...
			for (var d = 0; d < a.length; d++) {
				...
				for (var p = 0; p < n.length; p++) {
					if (n[p].status === true) {
						...
						} else if (n[p].enabledKey === "SLOC Desc") {
							c = c + "Storage Location Desc" + '","'
					...
					for (var p = 0; p < n.length; p++) {
						if (n[p].status === true) {
							if (n[p].enabledKey === "Batch – Batch") {
								...
								f = f + a[d].OutputToBatchOutput.results[g].storelocdesc + '","'
							} else if (n[p].enabledKey === "Batch – Reserved By") {
		...
		popOver: function (e) {
			...
			if (!this._oPopover) {
				r.load({
					name: "dksh.connectclient.InventoryStck.Fragments.popoverTable",
					controller: this
				}).then(function (e) {
					this._oPopover = e;
					this.getView().addDependent(this._oPopover);
		...
		navigateStkLot: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryStck.Fragments.StockLotPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		valueHelpRequestSalesOrg: function (e) {
			...
			if (t.salesOrgDataAccess === "No Access") {
				...
				if (!t.salesOrg) {
					t.salesOrg = sap.ui.xmlfragment("dksh.connectclient.InventoryStck.Fragments.SalesOrg", t);
		...
		StockLotSearch: function () {
			...
			var n;
			var d;
			//e._getPersonalizationDetails("keyStkLotView"); jj
